...
def Mostrar():
    ...
    if colecb.get() == 'factura':
        ...
        def MostrarFacturas():
            ...
            filtro2 = {}
            if preciott.get() != '':
                filtro.update({'Precio Total': int(preciott.get())})
            if fechat.get() != '':
    ...
    if colecb.get() == 'envio':
        ...
        def MostrarEnvios():
            ...
            if nombreprodt.get() != '':
                filtro.update({'Stock Enviado.Nombre Producto': nombreprodt.get()})
            if cantidadt.get() != '':
    ...
    if colecb.get() == 'empleado':
        ...
        def IngresarFactura():
            ...
            try:
                ...
                messagebox.showerror('Error', 'Los datos no se han ingresado correctamente')
...
def EliminarDatos():
    ...
    if colecb.get() == 'producto': 
        ...
        #Eliminar facturas
    if colecb.get() == 'factura': 
        nombrel = ttk.Label(frame2, text='Precio Total:')
        nombrel.grid(column=0, row=0, padx=5, pady=5)
        nombret = ttk.Entry(frame2)
        nombret.grid(column=1, row=0, padx=5, pady=5)

        marcal = ttk.Label(frame2, text='Fecha Factura:')
        marcal.grid(column=0, row=1, padx=5, pady=5)
        marcat = ttk.Entry(frame2)
        marcat.grid(column=1, row=1, padx=5, pady=5)

        preciol = ttk.Label(frame2, text='Nombre Empleado:')
        preciol.grid(column=0, row=2, padx=5, pady=5)
        preciot = ttk.Entry(frame2)
        preciot.grid(column=1, row=2, padx=5, pady=5)
        ...
        def Eliminar():
            documento = {
                'Precio Total': int(nombret.get()),
                'Fecha Factura' : marcat.get(),
                'Nombre Empleado' : preciot.get()
            }
            ...
            if resultado.deleted_count == 1:
                messagebox.showinfo("Eliminación exitosa", "La factura se eliminó exitosamente.")
                root2.destroy()
            else:
                messagebox.showerror("Error", "No se encontró la factura especificado.")
